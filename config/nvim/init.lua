...
-- Move by *screen* lines when no count is given; honor counts for real lines
vim.o.wrap = true -- actually wrap long lines
...
-- Don't show info message when opening without file
...
require("lazy").setup({
	...
	-- Comments (replacement for tpope/vim-commentary)
	{
	...
	{ -- Collection of various small independent plugins/modules
		...
		config = function()
			...
			end, { desc = "[C]lean up [W]hitespace" })
			...
			local function indent_info()
				...
				-- Go: tabs (gofmt conventions)
				if ft == "go" then
					return ("1 tab (%d columns)"):format(tabstop)
				end

				if vim.bo.expandtab then
					return ("%d spaces"):format(shift)
				else
					return ("1 tab (%d columns)"):format(tabstop)
				end
			end
			...
			statusline.section_fileinfo = function()
				-- filetype is empty sometimes (e.g. new buffer)
				local ft = vim.bo.filetype ~= "" and vim.bo.filetype or "text"
				return indent_info() .. " " .. ft
			end
