...
# Add percentage remaining to battery icon
defaults write com.apple.menuextra.battery ShowPercent -bool true

# Make buttons, menus, windows gray ??????????????????????????????????????????????????
...
# Clicking on scrollbar jumps to that spot instead of a page towards that direction ??????????????????????
...
# Disable the Mission Control gesture (swipe up with three fingers)
defaults write com.apple.dock showMissionControlGestureEnabled -int 0

# Disable the Desktop gesture (spread with thumb and three fingers)
...
# Use Zenburn theme by default in Terminal.app
...
defaults write com.??????????????????????????????????????????.Spectacle StartAtLogin -bool true

# Don’t show the preferences window on next start
defaults write com.??????????????????????????????????????????.Spectacle ShowPrefsOnNextStart -bool false

###############################################################################
# Seil.app                                                                    #
###############################################################################

# Enable remaping Caps Lock
defaults write org.pqrs.Seil sysctl -dict-add "enable_capslock" -bool true

# Remap Caps Lock to Escape
defaults write org.pqrs.Seil sysctl -dict-add "keycode_capslock" -int 53
...
# Hide Toolbar
defaults read owrite org.m0k.transmission "NSToolbar Configuration TRMainToolbar" -dict-add "TB Is Shown" -int 0
